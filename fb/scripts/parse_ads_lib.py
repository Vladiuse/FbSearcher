...
language = config.get('AdsLibParser', 'language')

keys = KeyWord.objects.filter(language_id=language).filter(number_in_dict__range=(start_key, end_key)).order_by('?')[:keys_count]
